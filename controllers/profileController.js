...
const updateSeekerProfile = async (req, res) => {
  try {
    ...
    // Get seeker ID safely from req.user, params, or body
    const seekerId = req.user?._id || req.params.id || req.body._id;
    ...
    const updatedSeeker = await JobSeeker.findByIdAndUpdate(seekerId, updateData, { new: true });
    ...
    res.status(500).json({ message: 'Error updating seeker profile', error: error.message });
  }
};
